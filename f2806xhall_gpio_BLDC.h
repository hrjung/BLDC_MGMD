...
#define HALL3_INIT_MACRO(v)																	\
	...
	/* Configure ECAP1/2/3 as GPIO-inputs (GPIO24/25/26) for Piccolo B*/					\
...
#define HALL3_DETERMINE_STATE_MACRO(v)														\
	...
	#define HALL3_INIT_MACRO(v)																	\
																							\
	EALLOW;                       /* Enable EALLOW*/										\
	/* Configure ECAP1/2/3 as GPIO-inputs (GPIO54/55/56) for Piccolo B*/					\
	GpioCtrlRegs.GPBMUX2.bit.GPIO54 = 0;  /* GPIO54 is ECAP1 - A */							\
	GpioCtrlRegs.GPBMUX2.bit.GPIO55 = 0;  /* GPIO55 is ECAP2 - B */							\
	GpioCtrlRegs.GPBMUX2.bit.GPIO56 = 0;  /* GPIO56 is ECAP3 - C */							\
																							\
	/* Config GPIO24/25/26 as inputs*/														\
	GpioCtrlRegs.GPBDIR.bit.GPIO54 = 0;   /* GPI554 is ECAP1 - A */							\
	GpioCtrlRegs.GPBDIR.bit.GPIO55 = 0;   /* GPI555 is ECAP2 - B */							\
	GpioCtrlRegs.GPBDIR.bit.GPIO56 = 0;   /* GPI556 is ECAP3 - C */							\
	EDIS;                         /* Disable EALLOW*/										\
																							\
	HALL3_DETERMINE_STATE_MACRO(v)															\
	v.HallGpioBuffer = v.HallGpio;       /* Init with current ECAP/GPIO logic levels*/		\
	...
	#define HALL3_DETERMINE_STATE_MACRO(v)														\
																								\
		/* temp.2-0 = GPIO56.GPIO55.GPIO54 */													\
		temp = (GpioDataRegs.GPBDAT.all>>22)&0x00000007; /* read all three GPIOs at once*/		\
																								\
		HallGpioBitA = (temp&0x00000001);			/* save GPIO54 - A*/						\
		HallGpioBitB = (temp&0x00000002)>>1;		/* save GPIO55 - B*/						\
		HallGpioBitC = (temp&0x00000004)>>2;		/* save GPIO56 - C*/						\
		HallGpioBitA = HallGpioBitA;/*<<2;				 shift GPIO54 - A*/						\
		HallGpioBitB = HallGpioBitB<<1;				/* shift GPIO55 - B*/						\
		HallGpioBitC = HallGpioBitC<<2;/*<<1;				 shift GPIO56 - C*/						\
																								\
		v.HallGpio = HallGpioBitA + HallGpioBitB + HallGpioBitC;

#define HALL3_INIT_MACRO_DRV8301HC(v)																	\
																							\
	EALLOW;                       /* Enable EALLOW*/										\
	/* Configure ECAP1/2/3 as GPIO-inputs (GPIO40/41/42) for Piccolo B*/					\
	GpioCtrlRegs.GPBMUX1.bit.GPIO40 = 0;  /* GPIO54 is ECAP1 - A */							\
	GpioCtrlRegs.GPBMUX1.bit.GPIO41 = 0;  /* GPIO55 is ECAP2 - B */							\
	GpioCtrlRegs.GPBMUX1.bit.GPIO42 = 0;  /* GPIO56 is ECAP3 - C */							\
																							\
	/* Config GPIO24/25/26 as inputs*/														\
	GpioCtrlRegs.GPBDIR.bit.GPIO40 = 0;   /* GPI554 is ECAP1 - A */							\
	GpioCtrlRegs.GPBDIR.bit.GPIO41 = 0;   /* GPI555 is ECAP2 - B */							\
	GpioCtrlRegs.GPBDIR.bit.GPIO42 = 0;   /* GPI556 is ECAP3 - C */							\
	EDIS;                         /* Disable EALLOW*/										\
																							\
	HALL3_DETERMINE_STATE_MACRO_DRV8301HC(v)															\
	v.HallGpioBuffer = v.HallGpio;       /* Init with current ECAP/GPIO logic levels*/		\
	v.HallGpioAccepted = v.HallGpio;     /* Init with current ECAP/GPIO logic levels*/
	/*----------------------------------------------------------------------------------------------
		 HALL3_DETERMINE_STATE Macro Definition
	...
	#define HALL3_DETERMINE_STATE_MACRO_DRV8301HC(v)														\
																								\
		/* temp.2-0 = GPIO42.GPIO41.GPIO40 */													\
		temp = (GpioDataRegs.GPBDAT.all>>8)&0x00000007; /* read all three GPIOs at once*/		\
																								\
		HallGpioBitA = (temp&0x00000001);			/* save GPIO40 - A*/						\
		HallGpioBitB = (temp&0x00000002)>>1;		/* save GPIO41 - B*/						\
		HallGpioBitC = (temp&0x00000004)>>2;		/* save GPIO42 - C*/						\
		HallGpioBitA = HallGpioBitA;/*<<2;				 shift GPIO40 - A*/						\
		HallGpioBitB = HallGpioBitB<<1;				/* shift GPIO41 - B*/						\
		HallGpioBitC = HallGpioBitC<<2;/*<<1;				 shift GPIO42 - C*/						\
																								\
		v.HallGpio = HallGpioBitA + HallGpioBitB + HallGpioBitC;
...
#define HALL3_INIT_MACRO_IMMB(v)																	\
																						\
EALLOW;                       /* Enable EALLOW*/										\
/* Configure ECAP1/2/3 as GPIO-inputs (GPIO54/55/56) for Piccolo B*/					\
GpioCtrlRegs.GPAMUX2.bit.GPIO24 = 0;  /* GPIO24 is ECAP1 - A */							\
...
GpioCtrlRegs.GPAMUX2.bit.GPIO26 = 0;  /* GPIO26 is ECAP3 - C */							\
																						\
/* Config GPIO24/25/26 as inputs*/														\
GpioCtrlRegs.GPADIR.bit.GPIO24 = 0;   /* GPI524 is ECAP1 - A */							\
GpioCtrlRegs.GPADIR.bit.GPIO25 = 0;   /* GPI525 is ECAP2 - B */							\
GpioCtrlRegs.GPADIR.bit.GPIO26 = 0;   /* GPI526 is ECAP3 - C */							\
EDIS;                         /* Disable EALLOW*/										\
																						\
HALL3_DETERMINE_STATE_MACRO_IMMB(v)															\
v.HallGpioBuffer = v.HallGpio;       /* Init with current ECAP/GPIO logic levels*/		\
v.HallGpioAccepted = v.HallGpio;     /* Init with current ECAP/GPIO logic levels*/
/*----------------------------------------------------------------------------------------------
	 HALL3_DETERMINE_STATE Macro Definition
...
#define HALL3_DETERMINE_STATE_MACRO_IMMB(v)														\
																							\
	/* temp.2-0 = GPIO56.GPIO55.GPIO54 */													\
	temp = (GpioDataRegs.GPADAT.all>>24)&0x00000007; /* read all three GPIOs at once*/		\
																							\
...
#define HALL3_DEBOUNCE_MACRO(v)																				\
																											\
	if (gdsSystem.u16BoardType == B_IMMB) { 	/* BOOST BOARD */											\
		HALL3_DETERMINE_STATE_MACRO_IMMB(v)	/* read HallGpio*/											    \
	} else if (gdsSystem.u16BoardType == B_DRV8301HC) { 	/* DRV8301HC   */								\
		HALL3_DETERMINE_STATE_MACRO_DRV8301HC(v)	/* read HallGpio*/										\
	} else {										/* BOOST BOARD */										\
		HALL3_DETERMINE_STATE_MACRO(v)	        															\
	}																										\
																											\
