...
History:
-------------------------------------------------------------------------------------
 03-09-2011	Version 1.0: 
------------------------------------------------------------------------------------*/

#ifndef __F2806X_BLDCPWM_H__
#define __F2806X_BLDCPWM_H__
...
------------------------------------------------------------------------------*/
#if(0)
#define F2806X_BLDC_PWM_GEN   {0x0000, \
                                0x7FFF, \
                                0x1D4C, \
                                0x0000, \
                                0x0001, \
                             }
#define PWMGEN_DEFAULTS 	F2806X_BLDC_PWM_GEN
#else
#define F2806X_BLDC_PWM_GEN   {0x0000, \
                                0x7FFF, \
                                0x1D4C, \
...
/* State s2: current flows to motor windings from phase A->C, de-energized phase = B					*/\
    ...
    {												/*													*/\
      EPwm1Regs.AQCSFRC.bit.CSFB = 0;       		/* Forcing disabledd on output B of EPWM1   		*/\
      EPwm1Regs.AQCTLB.bit.CAU = 2;         		/* Set high when CTR = CMPA on UP-count				*/\
      EPwm1Regs.AQCTLB.bit.ZRO = 1;         		/* Set low when CTR = Zero							*/\
      EPwm1Regs.CMPA.half.CMPA = (int16)(Tmp>>15); 	/* PWM signal on output B of EPWM1 (Q15 -> Q0)  	*/\
      EPwm1Regs.AQCSFRC.bit.CSFA = 2;       		/* Forcing a continuous High on output A of EPWM1  	*/\
...
/* State s3: current flows to motor windings from phase B->C, de-energized phase = A					*/\
    ...
    {												/*													*/\
      ...
      EPwm2Regs.AQCTLB.bit.CAU = 2;         		/* Set high when CTR = CMPA on UP-count				*/\
      EPwm2Regs.AQCTLB.bit.ZRO = 1;         		/* Set low when CTR = Zero							*/\
...
/* State s5: current flows to motor windings from phase C->A, de-energized phase = B					*/\
    ...
    {												/*													*/\
      ...
      EPwm3Regs.AQCTLB.bit.CAU = 2;         		/* Set high when CTR = CMPA on UP-count				*/\
      EPwm3Regs.AQCTLB.bit.ZRO = 1;         		/* Set low when CTR = Zero							*/\
...
/* Convert "DutyFunc" or "GPR0_BLDC_PWM" (Q15) duty modulation function to Q0							*/\
    ...
    EPwm1Regs.CMPB = (int16)(Tmp2>>15);				/* Sample in center of PWM pulse using CMPB			*/\
													/*													*/\
/* State s1: current flows to motor windings from phase B->A, de-energized phase = C					*/\
	if (v.CmtnPointer==0)							/*													*/\
    {												/*													*/\
      EPwm1Regs.AQCSFRC.bit.CSFA = 1;       		/* Forcing a continuous Low on output A of EPWM2   	*/\
	  EPwm1Regs.AQCSFRC.bit.CSFB = 2;       		/* Forcing a continuous High on output B of EPWM2   */\
													/*													*/\
	  EPwm2Regs.AQCSFRC.bit.CSFB = 0;       		/* Forcing disabled on output B of EPWM1	   		*/\
	  EPwm2Regs.AQCTLB.bit.CAU = 2;         		/* Set high when CTR = CMPA on UP-count				*/\
	  EPwm2Regs.AQCTLB.bit.ZRO = 1;         		/* Set low when CTR = Zero							*/\
	  EPwm2Regs.CMPA.half.CMPA = (int16)(Tmp>>15); 	/* PWM signal on output B of EPWM1 (Q15 -> Q0)		*/\
	  EPwm2Regs.AQCSFRC.bit.CSFA = 2;       		/* Forcing a continuous High on output A of EPWM1  	*/\
													/*													*/\
      EPwm3Regs.AQCSFRC.bit.CSFA = 1;       		/* Forcing a continuous Low on output A of EPWM3   	*/\
      EPwm3Regs.AQCSFRC.bit.CSFB = 1;       		/* Forcing a continuous Low on output B of EPWM3   	*/\
    }												/*													*/\
    												/*													*/\
/* State s2: current flows to motor windings from phase C->A, de-energized phase = B					*/\
    else if (v.CmtnPointer==1)  					/*													*/\
    {												/*													*/\
	  EPwm1Regs.AQCSFRC.bit.CSFA = 1;       		/* Forcing a continuous Low on output A of EPWM3   	*/\
	  EPwm1Regs.AQCSFRC.bit.CSFB = 2;       		/* Forcing a continuous High on output B of EPWM3   */\
													/*													*/\
      EPwm2Regs.AQCSFRC.bit.CSFA = 1;       		/* Forcing a continuous Low on output A of EPWM2   	*/\
      EPwm2Regs.AQCSFRC.bit.CSFB = 1;       		/* Forcing a continuous Low on output B of EPWM2   	*/\
													/*													*/\
	  EPwm3Regs.AQCSFRC.bit.CSFB = 0;       		/* Forcing disabledd on output B of EPWM1   		*/\
	  EPwm3Regs.AQCTLB.bit.CAU = 2;         		/* Set high when CTR = CMPA on UP-count				*/\
	  EPwm3Regs.AQCTLB.bit.ZRO = 1;         		/* Set low when CTR = Zero							*/\
	  EPwm3Regs.CMPA.half.CMPA = (int16)(Tmp>>15); 	/* PWM signal on output B of EPWM1 (Q15 -> Q0)  	*/\
	  EPwm3Regs.AQCSFRC.bit.CSFA = 2;       		/* Forcing a continuous High on output A of EPWM1  	*/\
    }												/*													*/\
													/*													*/\
/* State s3: current flows to motor windings from phase C->B, de-energized phase = A					*/\
    else if (v.CmtnPointer==2)  					/*													*/\
    {												/*													*/\
      EPwm1Regs.AQCSFRC.bit.CSFA = 1;       		/* Forcing a continuous Low on output A of EPWM1   	*/\
      EPwm1Regs.AQCSFRC.bit.CSFB = 1;       		/* Forcing a continuous Low on output B of EPWM1   	*/\
      ...
      EPwm2Regs.AQCSFRC.bit.CSFB = 2;       		/* Forcing a continuous High on output B of EPWM3   */\
      	  	  	  	  	  	  	  	  	  	  	  	/*													*/\
	  EPwm3Regs.AQCSFRC.bit.CSFB = 0;       		/* Forcing disabled on output B of EPWM2   			*/\
	  EPwm3Regs.AQCTLB.bit.CAU = 2;         		/* Set high when CTR = CMPA on UP-count				*/\
	  EPwm3Regs.AQCTLB.bit.ZRO = 1;         		/* Set low when CTR = Zero							*/\
	  EPwm3Regs.CMPA.half.CMPA = (int16)(Tmp>>15); 	/* PWM signal on output B of EPWM2 (Q15 -> Q0)  	*/\
	  EPwm3Regs.AQCSFRC.bit.CSFA = 2;       		/* Forcing a continuous High on output A of EPWM2  	*/\
    }												/*													*/\
   													/*													*/\
/* State s4: current flows to motor windings from phase A->B, de-energized phase = C					*/\
    else if (v.CmtnPointer==3)  					/*													*/\
    {												/*													*/\
	  EPwm1Regs.AQCSFRC.bit.CSFB = 0;       		/* Forcing disabled on output B of EPWM2   			*/\
	  EPwm1Regs.AQCTLB.bit.CAU = 2;         		/* Set high when CTR = CMPA on UP-count				*/\
	  EPwm1Regs.AQCTLB.bit.ZRO = 1;         		/* Set low when CTR = Zero							*/\
	  EPwm1Regs.CMPA.half.CMPA = (int16)(Tmp>>15); 	/* PWM signal on output B of EPWM2 (Q15 -> Q0)  	*/\
	  EPwm1Regs.AQCSFRC.bit.CSFA = 2;       		/* Forcing a continuous High on output A of EPWM2  	*/\
													/*													*/\
      EPwm2Regs.AQCSFRC.bit.CSFA = 1;       		/* Forcing a continuous Low on output A of EPWM1   	*/\
      EPwm2Regs.AQCSFRC.bit.CSFB = 2;       		/* Forcing a continuous High on output B of EPWM1   */\
													/*													*/\
      EPwm3Regs.AQCSFRC.bit.CSFA = 1;       		/* Forcing a continuous Low on output A of EPWM3   	*/\
      EPwm3Regs.AQCSFRC.bit.CSFB = 1;       		/* Forcing a continuous Low on output B of EPWM3   	*/\
    }												/*													*/\
													/*													*/\
/* State s5: current flows to motor windings from phase A->C, de-energized phase = B					*/\
    else if (v.CmtnPointer==4)  					/*													*/\
    {												/*													*/\
	  EPwm1Regs.AQCSFRC.bit.CSFB = 0;       		/* Forcing disabled on output B of EPWM3  			*/\
	  EPwm1Regs.AQCTLB.bit.CAU = 2;         		/* Set high when CTR = CMPA on UP-count				*/\
	  EPwm1Regs.AQCTLB.bit.ZRO = 1;         		/* Set low when CTR = Zero							*/\
	  EPwm1Regs.CMPA.half.CMPA = (int16)(Tmp>>15); 	/* PWM signal on output B of EPWM3 (Q15 -> Q0)  	*/\
	  EPwm1Regs.AQCSFRC.bit.CSFA = 2;       		/* Forcing a continuous High on output A of EPWM3  	*/\
													/*													*/\
      EPwm2Regs.AQCSFRC.bit.CSFA = 1;       		/* Forcing a continuous Low on output A of EPWM2   	*/\
      EPwm2Regs.AQCSFRC.bit.CSFB = 1;       		/* Forcing a continuous Low on output B of EPWM2   	*/\
													/*													*/\
	  EPwm3Regs.AQCSFRC.bit.CSFA = 1;       		/* Forcing a continuous Low on output A of EPWM1   	*/\
	  EPwm3Regs.AQCSFRC.bit.CSFB = 2;       		/* Forcing a continuous High on output B of EPWM1   */\
    }												/*													*/\
													/*													*/\
/* State s6: current flows to motor windings from phase B->C, de-energized phase = A					*/\
    else if (v.CmtnPointer==5)  					/*													*/\
    {												/*													*/\
      EPwm1Regs.AQCSFRC.bit.CSFA = 1;       		/* Forcing a continuous Low on output A of EPWM1   	*/\
      EPwm1Regs.AQCSFRC.bit.CSFB = 1;       		/* Forcing a continuous Low on output B of EPWM1   	*/\
													/*													*/\
	  EPwm2Regs.AQCSFRC.bit.CSFB = 0;       		/* Forcing disabled on output B of EPWM3  			*/\
	  EPwm2Regs.AQCTLB.bit.CAU = 2;         		/* Set high when CTR = CMPA on UP-count				*/\
	  EPwm2Regs.AQCTLB.bit.ZRO = 1;         		/* Set low when CTR = Zero							*/\
	  EPwm2Regs.CMPA.half.CMPA = (int16)(Tmp>>15); 	/* PWM signal on output B of EPWM3 (Q15 -> Q0)  	*/\
	  EPwm2Regs.AQCSFRC.bit.CSFA = 2;       		/* Forcing a continuous High on output A of EPWM3  	*/\
													/*													*/\
	  EPwm3Regs.AQCSFRC.bit.CSFA = 1;       		/* Forcing a continuous Low on output A of EPWM2   	*/\
	  EPwm3Regs.AQCSFRC.bit.CSFB = 2;       		/* Forcing a continuous High on output B of EPWM2   */\
    }

	#endif  // __F2806X_BLDCPWM_H__
